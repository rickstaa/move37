...
if __name__ == "__main__":
    ...
    exp_source = ptan.experience.ExperienceSourceFirstLast(
        env, agent, gamma=params["gamma"], steps_count=params["n_steps"]
    )
    buffer = common.PrioReplayBuffer(
        exp_source, params["replay_size"], params["prio_replay_alpha"]
    )
    ...
    with common.RewardTracker(writer, params["stop_reward"]) as reward_tracker:
        while True:
            ...
            beta = min(
                1.0,
                params["beta_start"]
                + frame_idx * (1.0 - params["beta_start"]) / params["beta_frames"],
            )

            new_rewards = exp_source.pop_total_rewards()
            if new_rewards:
                writer.add_scalar("beta", beta, frame_idx)
                finished, save_checkpoint = reward_tracker.reward(
                    new_rewards[0], frame_idx
                )
                if save_checkpoint:
                    torch.save(
                        net.state_dict(),
                        "./checkpoints/" + params["env_name"] + "-best.dat",
                    )
                if finished:
                    break
